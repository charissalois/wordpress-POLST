...
add_action('genesis_entry_header', 'polst_back_to_news', 1, 1);
function polst_back_to_news(){
	$uri = $_SERVER['QUERY_STRING'];
	$is_pro = $uri ; // for the first parameter
	if ( $is_pro == 'pro=1') {
		?>
		<a href="<?php echo esc_url( add_query_arg( 'pro', '1', get_permalink(2977)) )?>" class="button pink thin print-hide back-to-news">« Back to News</a>
		<?php
	} else {
		?>
		<a href="<?php echo get_permalink(2977)?>" class="button thin pink print-hide back-to-news">« Back to News</a>
		<?php
	}
}
